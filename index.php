...
?>
<?php 
  if(isset($_POST["btn-login"])) // voor het controleren of er iets is verzonden via het formulier
  {
      $email=$_POST["txt_uname_email"]; //moet nog beveiligd tegen sql-injectie
      $pass=$_POST["txt_password"];
      
	  														// wat doet strtolower en waarom is dit nodig?
	 
															  
	 // echo $query;
	 //hier is de query voor het controleren van je email en wachtwoord
	 $query = "select * from registration where email='$email' and password='$pass' LIMIT 1";
	  $result = mysqli_query($con,$query);
	  
	  
	  // tip: zet er commentaar bij dat uitlegt wat er staat en waarom het er staat
	  // dan onthoud je het veel beter
	  
	  
	  // echo "aantal:" . mysqli_num_rows($result);
     ...
     // selecteer de registratie voor controle 
      if(mysqli_num_rows($result) == 1)
      {
		  ...
		  // gaat terug naar de start pagina
